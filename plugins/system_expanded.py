...
class EnhancedMonitorPlugin(BasePlugin):
    ...
    def get_system_stats(self):
        """Throttle CPU load checks to avoid jitter at 10Hz"""
    ...
    def update(self):
        ...
        if self.page == 1: # OVERALL
            ...
            draw.text((2, 10), f"CPU: {self.cpu_load}%", font=self.font_med, fill=1)
            ...
            draw.text((2, 25), f"CPU: {self.cpu_temp}°C  ({self.cpu_load}%)", font=self.font_med, fill=1)
